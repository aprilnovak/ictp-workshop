...
[Problem]
  ...
  scaling = 100
  verbose = true
  temperature_blocks = '0 1 2 1001 3 1000'
  #density_blocks = '3 1000'
...
[Transfers]
  ...
  []
  [clad_surface_temperature_to_subchannel]
    type = MultiAppGeneralFieldNearestLocationTransfer
    from_multi_app = subchannel
    to_multi_app = conduction
  ...
  # this transfer assumes that the heat flux on the outer duct wall is zero (insulated)
  [duct_surface_temperature_to_conduction]
    type = MultiAppGeneralFieldNearestLocationTransfer
    to_multi_app = conduction
